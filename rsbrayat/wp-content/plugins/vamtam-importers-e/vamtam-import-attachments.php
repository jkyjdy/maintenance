...
class Vamtam_Import_Attachments extends WP_Background_Process {
	...
	protected function task( $task ) {
		...
		if ( is_array( $task ) && isset( $task['step'] ) ) {
			$data = $task['data'];
			...
			if ( $task['step'] === 1 ) {
				...
				if ( is_wp_error( $imported ) ) {
					$message = 'VAMTAM -- importing attachment failed:' . $data['attachment'][1] . " \n ";
					$message .='error: ' . $imported->get_error_message();
					$this->log( $message );

					return false;
				}

				$message = 'VAMTAM -- imported attachment:' . $data['attachment'][1] . " \n ";
				$this->log( $message );
				...
				$new_metadata = wp_generate_attachment_metadata( $data['post_id'], get_attached_file( $data['post_id'] ) );

				$message = 'VAMTAM -- updating attachment ' . $data['post_id'] . get_attached_file( $data['post_id'] ) . " \n ";
				$message .='result: ' . (int)wp_update_attachment_metadata( $data['post_id'], $new_metadata );
