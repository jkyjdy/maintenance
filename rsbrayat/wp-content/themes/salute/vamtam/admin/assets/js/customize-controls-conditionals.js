...
/* jshint esnext:true */
(function ($, undefined) {
  'use strict'; // Funcs shared across common vamtam controls. Common conventions for the html apply.
    ...
    controls: {
      ...
      addInputsChangeHandlers: function addInputsChangeHandlers(control, value) {
        ...
        control.container.on('change', '.resp-control input, .base-control.input-control input', function () {
          ...
          var newVal = $(this).val(); // Check UI type
      ...
      addRespBtnHandlers: function addRespBtnHandlers(control, value) {
        control.container.on('click', '.resp-btns [data-device]', function () {
          ...
          var values = $(options).siblings('.values');
          var ui_type = $(values).attr('data-type');
          var val, targets, unitVal, unitForDevice; // Check UI type
          ...
          $(targets).each(function (i, target) {
            val = $(target).attr('data-value'); // Update field value for device.
      ...
      addColorPickerHandlers: function addColorPickerHandlers(control, value) {
        ...
        var $pickers = control.container.find('resp-control .vamtam-color-picker, .base-control.color-picker-control .vamtam-color-picker'); // Bind color change.
      ...
      addSelectiveFontLoadingToSelect2: function addSelectiveFontLoadingToSelect2(control, select2) {
        ...
        control.enqueuedFonts = [], control.$previewContainer = null; // Load selected font.

        control.enqueueFont($(control.selector).find('.select2-selection__rendered[role="textbox"] > span').text());
        select2.on('select2:open', function () {
          control.$previewContainer = $(control.selector).find('.select2-results__options[role="listbox"]:visible'); // Load initial.
  ...
  var api = wp.customize; // toggle visibility of some controls based on a setting's value
