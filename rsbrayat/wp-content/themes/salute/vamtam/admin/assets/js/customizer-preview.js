      ...
      var change_handler_by_type = {
        number: function number(to) {
          ...
          if (VAMTAM_CUSTOMIZE_PREVIEW.percentages.indexOf(id) !== -1) {
            to += '%';
          } else if (VAMTAM_CUSTOMIZE_PREVIEW.numbers.indexOf(id) !== -1) {// as is
          } else {
            to += 'px';
          }

          document.documentElement.style.setProperty("--vamtam-".concat(id), to); // trigger a resize event if we change any dimension
