...
class VamtamPurchaseHelper extends VamtamAjax {
	...
	private static function server_tests() {
		...
		$tests = array(
			array(
				'name'  => esc_html__( 'PHP Version', 'salute' ),
				'test'  => version_compare( phpversion(), '5.5', '<' ),
				'value' => phpversion(),
				'desc'  => esc_html__( 'While this theme works with all PHP versions supported by WordPress Core, PHP versions 5.5 and older are no longer maintained by their developers. Consider switching your server to PHP 5.6 or newer.', 'salute' ),
			),
			array(
				'name'  => esc_html__( 'PHP Time Limit', 'salute' ),
				'test'  => $timeout > 0 && $timeout < 30,
				'value' => $timeout,
				'desc'  => esc_html__( 'The PHP time limit should be at least 30 seconds. Note that in some configurations your server (Apache/nginx) may have a separate time limit. Please consult with your hosting provider if you get a time out while importing the demo content.', 'salute' ),
			),
			array(
				'name'  => esc_html__( 'PHP Memory Limit', 'salute' ),
				'test'  => (int) $memory > 0 && $memoryB < 96 * 1024 * 1024,
				'value' => $memory,
				'desc'  => esc_html__( 'You need a minimum of 96MB memory to use the theme and the bundled plugins. For non-US English websites you need a minimum of 128MB in order to accomodate the translation features which are otherwise disabled.', 'salute' ),
			),
			array(
				'name'  => esc_html__( 'PHP ZipArchive Extension', 'salute' ),
				'test'  => ! class_exists( 'ZipArchive' ),
				'value' => '',
				'desc'  => esc_html__( 'ZipArchive is a requirement for importing the demo sliders.', 'salute' ),
			),
		);
	...
	public static function tgmpa_plugins_complete_actions( $update_actions, $plugin_info ) {
		if ( isset( $update_actions['dashboard'] ) ) {
			$update_actions['dashboard'] = sprintf(
				esc_html__( 'All plugins installed and activated successfully. %1$s', 'salute' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=vamtam_theme_setup_import_content' ) ) . '" class="button button-primary">' . esc_html__( 'Continue with theme setup.', 'salute' ) . '</a>'
			);
	...
	public static function admin_menu() {
		add_menu_page( esc_html__( 'VamTam', 'salute' ), esc_html__( 'VamTam', 'salute' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ), '', 2 );
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Dashboard', 'salute' ), esc_html__( 'Dashboard', 'salute' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ) );
	...
	public static function admin_menu_1() {
		...
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Import Demo Content', 'salute' ), esc_html__( 'Import Demo Content', 'salute' ), 'edit_theme_options', 'vamtam_theme_setup_import_content', array( __CLASS__, 'vamtam_theme_setup_import_content' ) );
	}
	...
	public static function registration_warning() {
		...
		<div class="vamtam-notice-wrap">
			<div class="vamtam-notice">
				<p>
					<?php echo sprintf( esc_html__( 'The %s theme needs to be registered.', 'salute' ), $theme_name ); ?>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=vamtam_theme_setup' ) ); ?>">
						<?php echo esc_html__( 'Register Now', 'salute' ); ?>
					</a>
				</p>
			</div>
		</div>
	...
	public static function admin_init() {
		...
		add_settings_field(
			...
			esc_html__( 'Enter your purchase code from ThemeForest', 'salute' ),
		...
		<div id="vamtam-ts-homepage" class="vamtam-ts">
			...
			<div id="vamtam-ts-main">
							...
							<form id="vamtam-register-form" method="post" action="options.php" autocomplete="off">
								<?php if ( $valid_key ) : ?>
									<div id="vamtam-verified-code">
										<p>
											<?php  esc_html_e( 'Thanks for the verification!', 'salute' ) ?>
											<br />
											<?php echo esc_html( sprintf( __( 'You can now enjoy %s and build great websites.', 'salute' ) , $theme_name ) ); ?>
										</p>
									</div>
								<?php else : ?>
				...
				<div id="vamtam-check-license-disclaimer">
					<h5><?php esc_html_e( 'Licensing Terms', 'salute' ); ?></h5>
					<p>
						<?php esc_html_e( 'Please be advised, in order to use the theme in a legal manner, you need to purchase a separate license for each domain you are going to use the theme on. A single license is limited to a single domain/application. For more information please refer to the license included with the theme or ', 'salute' ); ?>
						<a href="http://themeforest.net/licenses" target="_blank">
							<?php esc_html_e( 'Licensing Terms', 'salute' ); ?>
						</a>
						<?php esc_html_e( ' on the ThemeForest site.', 'salute'); ?>
					</p>
				</div>
						...
						<div id="vamtam-server-tests">
							<h3>
								<?php if ( $status['fail'] > 0 ) : ?>
									<?php esc_html_e( 'System Status', 'salute' ) ?>
									<?php $fail = $status['fail']; ?>
									<small><?php printf( esc_html( _n( '(%d potential issue)', '(%d potential issues)', $fail, 'salute' ) ), $fail ) ?></small>
								<?php endif ?>
							</h3>
						</div>
					<?php endif ?>
	...
	public static function dashboard_navigation()
	{
		...
		<nav id="vamtam-ts-nav-menu">
			<div id="vamtam-theme-title">
				<span id="vamtam-ts-greeter"><?php esc_html_e( 'WELCOME TO', 'salute' ); ?></span>
				<span id="vamtam-ts-greeter-title"><?php echo esc_html( $theme_name ); ?></span>
				<span id="vamtam-ts-greeter-ver"><?php echo sprintf( esc_html__( 'VER. %s', 'salute' ), $theme_version ); ?></span>
			</div>
			<ul>
				<li class="<?php echo esc_attr( $cur_route === 'toplevel_page_' . $routes[0] ? 'is-active' : '' ); ?>" >
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[0] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Register' , 'salute' ); ?></span>
				...
				<?php if ( isset( $tgmpa_instance ) && isset( $tgmpa_instance->page_hook ) ) : ?>
					<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[1] ? 'is-active' : '' ); ?>" >
						<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[1] ) ); ?>">
							...
							<span><?php echo esc_html__( 'Install Plugins' , 'salute' ); ?></span>
				...
				<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[2] ? 'is-active' : '' ); ?>" >
					<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[2] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Import Demo' , 'salute' ); ?></span>
			...
			<a id="vamtam-hs-btn" class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[3] ? 'is-active' : ''); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[3] ) ); ?>">
				...
				<span><?php echo esc_html__( 'Help & Support' , 'salute' ); ?></span>
			</a>
		</nav>
		<?php
	}
	...
	public static function import_buttons() {
		...
		$messages = array(
			'success-msg' => esc_html__( 'Imported.', 'salute' ),
			'error-msg  ' => esc_html__( 'Failed to import. Please <a href="{fullimport}" target="_blank">click here</a> in order to see the full error message.', 'salute' ),
		);

		$import_tests = array(
			array(
				'test'   => defined( 'ELEMENTOR_PRO__FILE__' ),
				'title'  => esc_html__( 'Posts, Pages and Site Layout', 'salute' ),
				'failed' => wp_kses( __( "This theme requires Elementor Pro. If you don't have Elementor Pro please download the <a href='https://proelements.github.io/proelements.org/' target='_blank'>ProElements plugin available here</a>. Install and activate it, and then proceed with importing the demo content. If you have any issues with the importer please <a href='https://elementor.support.vamtam.com/support/login' target='_blank'>open a ticket here</a>.", 'salute' ), 'vamtam-a-span' ),
			),
		);
		...
		$img_progress = $total_attachements > 0 && class_exists( 'Vamtam_Importers_E' ) && is_callable( [ 'Vamtam_Importers_E', 'get_attachment_progress' ] ) ?
			Vamtam_Importers_E::get_attachment_progress()['text'] :
			esc_html__( 'checking...', 'salute' );
		...
		$buttons = array(
			array(
				'label'          => esc_html__( 'Dummy Content Import', 'salute' ),
				'id'             => 'content-import-button',
				'description'    => esc_html__( 'You are advised to use this importer only on new WordPress sites.', 'salute' ),
				'button_title'   => esc_html__( 'Import', 'salute' ),
				...
				'data'           => array_merge( $messages, [
					...
					'success-msg'      => sprintf( esc_html__( 'Main content imported. Image import progress: <span class="vamtam-image-import-progress">%s</span>.', 'salute' ), $img_progress ),
					'fail-msg'         => esc_html__( 'Failed to import - error {statuscode}. <a href="{fullimport}" target="_blank">Click here</a> in order to see the full error message returned by your server. We recommend that you contact your hosting provider for advice, as solving this issue is often specific to each server.', 'salute' ),
					'timeout-msg'      => esc_html__( 'Failed to import. This is most likely caused by a timeout. Please contact your hosting provider for advice as to how you can increase the time limit on your server.', 'salute' ),
				] ),
				'additional_msg' => $import_disabled_msg . wp_kses( __( 'Images will be downloaded in the background after the main import is complete. Depending on your server, this may take several minutes.<br> In the meantime you may notice that some images are not visible.', 'salute' ), [ 'ul' => ['id'], 'br' ] ),
				'disabled_msg_plain' => '',
			),
		);
	...
	public static function purchase_key( $args ) {
		...
		if ( ! defined( 'ENVATO_HOSTED_SITE' ) ) {
			...
			if ( ! $valid_key ) {
				echo '<p id="vamtam-code-help">' . wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where can I find my Item Purchase Code?</a>', 'salute' ), 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ), 'vamtam-a-span' ) . '</p>';
				...
				echo '>' . esc_html__( 'Register', 'salute' );
				echo '</button>';
				...
				echo esc_html__( 'Continue to required plguins', 'salute' );
				echo '</a>';
				...
				echo esc_html__( 'Continue to demo import', 'salute' );
				echo '</a>';
			}
